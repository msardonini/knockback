...
static int lis3dh_write_reg(const uint8_t reg,
                            const uint8_t value);
...
int lis3dh_read_aux_adc1(int16_t *out)
{
    return lis3dh_read_regs(LIS3DH_REG_OUT_AUX_ADC1_L,
                            LIS3DH_ADC_DATA_SIZE, (uint8_t *)out);
}

int lis3dh_read_aux_adc2(int16_t *out)
{
    return lis3dh_read_regs(LIS3DH_REG_OUT_AUX_ADC2_L,
                            LIS3DH_ADC_DATA_SIZE, (uint8_t *)out);
}

int lis3dh_read_aux_adc3(int16_t *out)
{
    return lis3dh_read_regs(LIS3DH_REG_OUT_AUX_ADC3_L,
                            LIS3DH_ADC_DATA_SIZE, (uint8_t *)out);
}

int lis3dh_set_aux_adc(const uint8_t enable,
                       const uint8_t temperature)
{
    return lis3dh_write_bits(LIS3DH_REG_TEMP_CFG_REG,
                             LIS3DH_TEMP_CFG_REG_ADC_PD_MASK,
                             (enable ? LIS3DH_TEMP_CFG_REG_ADC_PD_MASK : 0) |
                             (temperature ? LIS3DH_TEMP_CFG_REG_TEMP_EN_MASK : 0));
}

int lis3dh_set_axes(const uint8_t axes)
{
    return lis3dh_write_bits(LIS3DH_REG_CTRL_REG1,
                             LIS3DH_CTRL_REG1_XYZEN_MASK, axes);
}

int lis3dh_set_fifo(const uint8_t mode, const uint8_t watermark)
{
    int status;
    uint8_t reg;
    ...
    reg |= mode;
    status = lis3dh_write_reg(LIS3DH_REG_FIFO_CTRL_REG, reg);
    if (status < 0) {
        /* communication error */
        return status;
    }
    if (mode != 0x00) {
        status = lis3dh_write_bits(LIS3DH_REG_CTRL_REG5,
...
int lis3dh_set_odr(const uint8_t odr)
{
    return lis3dh_write_bits(LIS3DH_REG_CTRL_REG1,
        LIS3DH_CTRL_REG1_ODR_MASK, odr);
}

int lis3dh_set_scale(const uint8_t scale)
{
    uint8_t scale_reg;
    ...
    return lis3dh_write_bits(LIS3DH_REG_CTRL_REG4,
                             LIS3DH_CTRL_REG4_FS_MASK, scale_reg);
}

int lis3dh_set_int1(const uint8_t mode)
{
    return lis3dh_write_reg(LIS3DH_REG_CTRL_REG3, mode);
}
...
int lis3dh_get_fifo_level()
{
    ...
    if (lis3dh_read_regs(LIS3DH_REG_FIFO_SRC_REG, 1, &reg) != 0) {
        return -1;
    }
 ...
 * @return                  -1 on error
 */
static int lis3dh_write_reg(const uint8_t reg, const uint8_t value)
{
